...
public class DemoScene : MonoBehaviour {
	...
	void Start () {
		...
		_client1.OnTopicMessage += (sender, e) => {
			_outputText = Encoding.UTF8.GetString(e.Message.Data);
		};

		_client2.OnTopicMessage += (sender, e) => {
			_outputText = Encoding.UTF8.GetString(e.Message.Data);
		};
	}
	...
	public void Player1RegisterClick() {
		...
		_client1.Register(request, session => {
	...
	public void Player1LoginClick() {
		string id = SystemInfo.deviceUniqueIdentifier;
		var request = NAuthenticateMessage.Device(id);
		_client1.Login(request, session => {
	...
	public void Player2RegisterClick() {
		...
		_client2.Register(request, session => {
	...
	public void Player2LoginClick() {
		var request = NAuthenticateMessage.Email(Email, Password);
		_client2.Login(request, session => {
	...
	public void Player1GetSelf() {
		_client1.Send(NSelfFetchMessage.Default(), result => {
			Debug.LogFormat ("Player 1 handle: '{0}'.", result.Handle);
			_idPlayer1 = result.Id;
	...
	public void Player2GetSelf() {
		_client2.Send(NSelfFetchMessage.Default(), result => {
			Debug.LogFormat ("Player 2 handle: '{0}'.", result.Handle);
			_idPlayer2 = result.Id;
	...
	public void Player1JoinTopic() {
		NTopicJoinMessage topicJoin = new NTopicJoinMessage.Builder ().TopicDirectMessage (_idPlayer2).Build ();
		_client1.Send (topicJoin, topic => {
	...
	public void Player2JoinTopic() {
		NTopicJoinMessage topicJoin = new NTopicJoinMessage.Builder ().TopicDirectMessage (_idPlayer1).Build ();
		_client2.Send (topicJoin, topic => {
	...
	public void Player1SendChatMessage() {
		...
		NTopicMessageSendMessage msg = NTopicMessageSendMessage.Default(_topicPlayer1, data);
		_client1.Send(msg, ack => {
	...
	public void Player2SendChatMessage() {
		...
		NTopicMessageSendMessage msg = NTopicMessageSendMessage.Default (_topicPlayer2, data);
		_client2.Send (msg, ack => {
